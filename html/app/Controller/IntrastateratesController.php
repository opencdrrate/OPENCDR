<?php
class IntrastateratesController extends AppController {

	var $name = 'Intrastaterates';
	var $uses = array('Intrastateratemaster');
	
	function index($customerid = null) {
		$this->Intrastateratemaster->recursive = 0;
		if(!empty($customerid)){
		...
		$this->set('intrastateratemasters', $this->paginate());
	}
	...
	function delete($id = null,$customerid = null) {
		...
		if ($this->Intrastateratemaster->delete($id)) {
	...
	function tocsv($customerid = null){
		if(!empty($customerid)){
			$this->set('data', $this->Intrastateratemaster->find('all', 
				array('conditions' => 
	...
	function import($customerid = null){
		$this->set('customerid', $customerid);
		if (!empty($this->data)) {
			$customerid = $this->data['Intrastateratemaster']['customerid'];
			...
			if($error == 1){
				...
				$messages = $this->Intrastateratemaster->import($filename, $customerid);
